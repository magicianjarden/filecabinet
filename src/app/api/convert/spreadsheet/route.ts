...
import { spreadsheetConverter } from '@/lib/converters/spreadsheet';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!spreadsheetConverter.inputFormats.includes(inputFormat)) {
      return NextResponse.json({
    ...
    if (!spreadsheetConverter.outputFormats.includes(outputFormat)) {
      return NextResponse.json({
    ...
    const result = await handleConversionWithStats(
      ...
      async () => {
        ...
        return await spreadsheetConverter.convert(buffer, inputFormat, outputFormat);
      }
    );
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to convert spreadsheet' },
      { status: 500 }
    );
  }
} 